...
function newsletter_campaign_kit_get_provider_defaults() {
	$brevo_key       = defined( 'NEWSLETTER_CAMPAIGN_KIT_BREVO_API_KEY' ) ? trim( (string) NEWSLETTER_CAMPAIGN_KIT_BREVO_API_KEY ) : trim( (string) getenv( 'NEWSLETTER_CAMPAIGN_KIT_BREVO_API_KEY' ) );
	$default_provider = '' !== $brevo_key ? 'brevo' : 'wp_mail';
...
function newsletter_campaign_kit_get_provider_settings() {
	...
	$settings['provider']   = in_array( $settings['provider'], array( 'wp_mail', 'brevo', 'resend', 'http_api', 'external_filter' ), true ) ? $settings['provider'] : 'wp_mail';
...
function newsletter_campaign_kit_save_provider_settings() {
	...
	$settings   = array(
		'provider'          => in_array( $provider, array( 'wp_mail', 'brevo', 'resend', 'http_api', 'external_filter' ), true ) ? $provider : 'wp_mail',
				...
				<tr>
					...
					<td>
						<select id="provider" name="provider">
							...
							<option value="brevo" <?php selected( $settings['provider'], 'brevo' ); ?>><?php esc_html_e( 'Brevo API (recommended)', 'newsletter-campaign-kit' ); ?></option>
							<option value="resend" <?php selected( $settings['provider'], 'resend' ); ?>><?php esc_html_e( 'Resend API', 'newsletter-campaign-kit' ); ?></option>
						...
						<p class="description"><?php esc_html_e( 'Brevo and Resend use fixed HTTPS endpoints and server-side API keys. wp_mail keeps the WordPress SMTP stack available for local and custom installations.', 'newsletter-campaign-kit' ); ?></p>
					</td>
				</tr>
				<tr>
					<th scope="row"><?php esc_html_e( 'Native provider readiness', 'newsletter-campaign-kit' ); ?></th>
					<td>
						<p><strong>Brevo:</strong> <?php echo esc_html( ! empty( $native_status['brevo'] ) ? __( 'Ready', 'newsletter-campaign-kit' ) : __( 'API key missing', 'newsletter-campaign-kit' ) ); ?></p>
						<p><strong>Resend:</strong> <?php echo esc_html( ! empty( $native_status['resend'] ) ? __( 'Ready', 'newsletter-campaign-kit' ) : __( 'API key missing', 'newsletter-campaign-kit' ) ); ?></p>
						<p class="description"><?php esc_html_e( 'Define NEWSLETTER_CAMPAIGN_KIT_BREVO_API_KEY or NEWSLETTER_CAMPAIGN_KIT_RESEND_API_KEY in the server environment. Secret values are never stored or displayed.', 'newsletter-campaign-kit' ); ?></p>
					</td>
				</tr>
				<tr>
