...
function newsletter_campaign_kit_get_blocks( $include_archived = false ) {
	global $wpdb;
	...
	if ( $include_archived ) {
		return $wpdb->get_results( "SELECT * FROM {$table} ORDER BY category ASC, updated_at DESC LIMIT 200", ARRAY_A );
	}

	return $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$table} WHERE status = %s ORDER BY category ASC, name ASC LIMIT 200", 'active' ), ARRAY_A );
}
...
function newsletter_campaign_kit_render_blocks_page() {
	...
	$blocks     = newsletter_campaign_kit_get_blocks( true );
	$categories = newsletter_campaign_kit_get_block_categories();
	?>
	<div class="wrap newsletter-campaign-kit-admin">
		...
		<table class="widefat striped"><thead><tr><th><?php esc_html_e( 'Name', 'newsletter-campaign-kit' ); ?></th><th><?php esc_html_e( 'Category', 'newsletter-campaign-kit' ); ?></th><th><?php esc_html_e( 'Status', 'newsletter-campaign-kit' ); ?></th><th><?php esc_html_e( 'Actions', 'newsletter-campaign-kit' ); ?></th></tr></thead><tbody>
